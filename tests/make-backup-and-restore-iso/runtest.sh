...
POST_RECOVERY_SCRIPT=(\"ls -R /mnt/local/$(dirname $TMT_PLAN_DATA )/discover > /dev/console 2>&1;\" \"tail /var/lib/rear/restore/recover.backup.*.restore.log > /dev/ttyS0 2>&1;\" \"findmnt > /dev/ttyS0 2>&1;\" \"mount > /dev/ttyS0 2>&1;\" \"sleep 900;\")
...
BTRFS_SUBVOLUME_GENERIC_SETUP=(/dev/nvme0n1p4 /dev/nvme0n1p5)
